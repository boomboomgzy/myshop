...
class Order(ExtendModel):
    ...
    total_amount=models.IntegerField(max_digits=10,decimal_places=2,verbose_name='商品总金额')
    send_price=models.DecimalField(max_digits=10,decimal_places=2,verbose_name='运费')
    pay_method=models.SmallIntegerField(choices=ORDER_PAY_METHOD,default=1,verbose_name='支付方式')
    status=models.SmallIntegerField(choices=ORDER_STATU,default=1,verbose_name='订单状态')
...
class OrderSku(ExtendModel):
    ...
    order=models.ForeignKey(Order,related_name='order_skus',on_delete=models.CASCADE,verbose_name='所属订单')
