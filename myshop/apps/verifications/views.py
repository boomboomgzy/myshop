...
class SmsCodeView(View):
    ...
    def get(self,req,mobile):
        ...
        #检查用户是否频繁发送验证码
        sms_send_flag_key=Rediskey.SMS_SEND_FLAG_KEY.format(mobile)
        send_flag=redis_cli.get(sms_send_flag_key)
        if send_flag:
            res=R.set_result(StatusCodeEnum.THROTTLING_ERR).data()

        #获取图片验证码
        img_code_key=Rediskey.IMG_CODE_KEY.format(uuid)
        img_code_correct=redis_cli.get(img_code_key).decode()       
        #判断redis中该图片验证码是否存在
        if img_code_correct is not None:
