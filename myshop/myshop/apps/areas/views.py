        ...
        if area_id is not None:
            ...
            if sub_areas_list is None:
                try:
                    ...
                    #将其存入缓存 有效时间 单位:s
                    cache.set(Rediskey.SUB_AREAS_LIST_KEY.format(area_id=area_id),formatted_sub_areas_list,MYSHOP_REDIS_AREAS_EXPIRES)
                    
                    res=R.ok().data(**{
                        'sub_areas_list':formatted_sub_areas_list
                    })
            ...
            if provinces_list is None:
                ...
                try:
                    ...
                    res=R.ok().data(**{
                        'province_list': formatted_provinces_list
                    })
                    return JsonResponse(res)           
            #缓存中有相应数据
            else:
