let vm = new Vue({
    ...
    methods: {
                ...
                axios.get(url, {
                    ...
                    .then(response => {
                        ...
                        for (let i = 0; i < this.hot_skus.length; i++) {
                            this.hot_skus[i].url = '/goods/detail/' + this.hot_skus[i].id + '/';
                        }
                    })
                    .catch(error => {
        ...
        goods_visit_count() {
            if (this.category_id) {
                let url = '/goods/detail/visit/' + this.category_id + '/';
                axios.post(url, {}, {
                    headers: {
        ...
        get_goods_comment() {
            if (this.sku_id) {
                let url = '/orders/comment/' + this.sku_id + '/';
                axios.get(url, {
                    responseType: 'json'
                })
                    .then(response => {
                        this.comments = response.data.data.comment_list;
