let vm = new Vue({
                ...
                axios.get(url, {
                    ...
                    .then(response => {
                        if (response.data.code == '0') {
                            this.cities = response.data.data.sub_areas_list;
                        } else {
        ...
        'form_address.city_id': function(){
            if (this.form_address.city_id){
                ...
                axios.get(url, {
                    ...
                    .then(response => {
                        if (response.data.code === 0) {
                            this.districts = response.data.data.sub_areas_list;
                        } else {
                            console.log(response.data);
                            this.districts = [];
                        }
    ...
    methods: {
        ...
        show_add_site(){
            ...
            this.form_address.province_name = '';
            this.form_address.city_name = '';
            this.form_address.district_name = '';           
        ...
        show_edit_site(index){
            ...
            // 只获取要编辑的数据
            address_info=JSON.parse(JSON.stringify(this.addresses[index]));
            this.form_address=address_info;
        },
        // 校验收货人
        check_receiver(){
        ...
        get_provinces(){
            ...
            axios.get(url, {
                ...
                .then(response => {
                    if (response.data.code == '0') {
                        this.provinces = response.data.data.province_list;
                    } else {
                        console.log(response.data);
                        this.provinces = [];
                    }
            ...
            if (this.error_receiver || this.error_place || this.error_mobile || this.error_email || !this.form_address.province_id || !this.form_address.city_id || !this.form_address.district_id ) {
                ...
                if (this.editing_address_index === '') {
                    ...
                    axios.post(url, this.form_address, {
                        ...
                        .then(response => {
                            if (response.data.code === 0) {
                                ...
                                this.addresses.splice(0, 0, response.data.data.address);
                                if(response.data.data.default_address_id){
                                    this.default_address_id=response.data.data.default_address_id;
                                }
                                this.is_show_edit = false;
                            } else if (response.data.code === 4101) {
                                location.href = '/users/login/?next=/users/addresses/';
                            } else {
                    ...
                    axios.put(url, this.form_address, {
                        ...
                        .then(response => {
                            if (response.data.code === 0) {
                                this.addresses[this.editing_address_index] = response.data.data.address;
                                this.is_show_edit = false;
                            } else if (response.data.code === 4101) {
                                location.href = '/users/login/?next=/users/addresses/';
                            } else {
